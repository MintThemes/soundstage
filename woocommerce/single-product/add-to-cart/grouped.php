...
<form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="product-form cart" method="post" enctype='multipart/form-data'>
	
			<?php foreach ( $grouped_products as $child_product ) : ?>
				<div class="row">
					<div style="vertical-align: middle; line-height: 2.55em">
						<label for="product-<?php echo $child_product['product']->id; ?>"><?php
					...
					<?php elseif ( ! $quantites_required ) : ?>
						...
						<span class="button">
							<span class="ico"></span>
							<button type="submit" name="quantity[<?php echo $child_product['product']->id; ?>]" value="1" class="single_add_to_cart_button alt"><?php _e('Add to cart', 'woocommerce'); ?></button>
						</span>
